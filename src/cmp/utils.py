...
class ContainerSet:
    ...
    def items(self):
        return self.set.union({u'\N{GREEK SMALL LETTER EPSILON}'}
            if self.contains_epsilon else {})
    
    def __len__(self):
        return len(self.set) + int(self.contains_epsilon)

    def __str__(self):
        return f'{str(self.set)}'
...
class DisjointSet:
    ...
    @property
    def groups(self):
        return [[n for n in self.nodes.values() if n.representative == r]
